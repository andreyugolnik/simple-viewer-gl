...
CFormatDds::CFormatDds(Callback callback) : CFormat(callback) {
}
...
bool CFormatDds::Load(const char* filename, int subImage) {
	...
	uint32 mipMapCount	= (header.dwFlags & DDSD_MIPMAPCOUNT) ? header.dwMipMapCount : 1;
	//if( mipMapCount > 1 ) {
		//hasMipmaps_ = true;
	//}
	if(li->compressed) {
		...
		//for(uint32 ix = 0; ix < mipMapCount; ++ix) {
			//fread( data, 1, size, f );

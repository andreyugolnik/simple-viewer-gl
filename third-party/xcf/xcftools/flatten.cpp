...
static rgba __ATTRIBUTE__((noinline, const)) composite_one(rgba bot, rgba top)
{
 ...
 */
static Tile* __ATTRIBUTE__((noinline)) merge_normal(Tile* bot, Tile* top)
{
    unsigned i;
    assertTileCompatibility(bot, top);

    /* See if there is an easy winner */
    if ((bot->summary & TILESUMMARY_ALLNULL) || (top->summary & TILESUMMARY_ALLFULL))
    {
        freeTile(bot);
        return top;
    }
...
{
    return scaletable[bot][bot] + 2 * scaletable[top][scaletable[bot][255 - bot]];
...
static void RGBtoHSV(rgba rgb, HSV* hsv)
{
 ...
 */
static void __ATTRIBUTE__((noinline)) merge_exotic(Tile* bot, const Tile* top, GimpLayerModeEffects mode)
{
    unsigned i;
    assertTileCompatibility(bot, top);
    if ((bot->summary & TILESUMMARY_ALLNULL) != 0)
    {
        return;
    }
    if ((top->summary & TILESUMMARY_ALLNULL) != 0)
    ...
    case GIMP_##mode##_MODE:                                    \
        ...
        case GIMP_COLOR_MODE:
        {
            ...
            else
            {
                ...
                {
                    hsvBot.ch2 = hsvBot.ch1 + (mfNum * (hsvBot.ch3 - hsvBot.ch1) + mfDenom / 2) / mfDenom;
                }
            }
            switch (hsvBot.hue)
            {
#define HEXTANT(b, m, t)           \
    case HSV::HUE_##b##_##m##_##t: \
        b = hsvBot.ch1;            \
        m = hsvBot.ch2;            \
        t = hsvBot.ch3;            \
        break;
...
#undef HEXTANT
        ...
        {
            bot->pixels[i] = (bot->pixels[i] & (255 << ALPHA_SHIFT)) + (RED << RED_SHIFT) + (GREEN << GREEN_SHIFT) + (BLUE << BLUE_SHIFT);
        }
        else
        {
            ...
            */
            ...
            bot->pixels[i] = (bp & (255 << ALPHA_SHIFT)) + ((rgba)scaletable[tfrac][RED] << RED_SHIFT) + ((rgba)scaletable[tfrac][GREEN] << GREEN_SHIFT) + ((rgba)scaletable[tfrac][BLUE] << BLUE_SHIFT) + ((rgba)scaletable[255 ^ tfrac][255 & (bp >> RED_SHIFT)] << RED_SHIFT) + ((rgba)scaletable[255 ^ tfrac][255 & (bp >> GREEN_SHIFT)] << GREEN_SHIFT) + ((rgba)scaletable[255 ^ tfrac][255 & (bp >> BLUE_SHIFT)] << BLUE_SHIFT);
        }
    }
    return;
}

static void dissolveTile(Tile* tile)
{
    unsigned i;
    summary_t summary;
...
static void roundAlpha(Tile* tile)
{
    unsigned i;
    summary_t summary;
    assert(tile->refcount == 1);
 ...
 */
static Tile* flattenTopdown(FlattenSpec* spec, Tile* top, unsigned nlayers, const rect* where)
{
    Tile* tile;
...
static void addBackground(FlattenSpec* spec, Tile* tile, unsigned ncols)
{
    unsigned i;
    ...
    {
    case FlattenSpec::FORBID_PARTIAL_TRANSPARENCY:
        if (!(tileSummary(tile) & TILESUMMARY_CRISP))
        {
            FatalGeneric(102, _("Flattened image has partially transparent pixels"));
        }
        break;
    case FlattenSpec::DISSOLVE_PARTIAL_TRANSPARENCY:
        dissolveTile(tile);
        break;
    case FlattenSpec::ALLOW_PARTIAL_TRANSPARENCY:
    case FlattenSpec::PARTIAL_TRANSPARENCY_IMPOSSIBLE:
        break;
    }
    ...
    if (!FULLALPHA(spec->default_pixel))
    {
        return;
    }
...
void flattenIncrementally(FlattenSpec* spec, lineCallback callback)
{
    ...
    for (where.t = spec->dim.c.t; where.t < spec->dim.c.b; where.t = where.b)
    {
        ...
        for (y = 0; y < nrows; y++)
        {
            rows[y] = (rgba*)xcfmalloc(4 * (spec->dim.c.r - spec->dim.c.l));
        }
        ...
        for (where.l = spec->dim.c.l; where.l < spec->dim.c.r; where.l = where.r)
        {
            ...
            assert(toptile.summary == TILESUMMARY_UPTODATE + TILESUMMARY_ALLNULL + TILESUMMARY_CRISP);
...
rgba** flattenAll(FlattenSpec* spec)
{
    rgba** rows = (rgba**)xcfmalloc(spec->dim.height * sizeof(rgba*));
    if (verboseFlag)
    {
...
void shipoutWithCallback(FlattenSpec* spec, rgba** pixels, lineCallback callback)
{
    unsigned i;

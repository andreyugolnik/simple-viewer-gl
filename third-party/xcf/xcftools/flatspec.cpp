...
void init_flatspec(FlattenSpec* spec)
{
    ...
    spec->out_color_mode = FlattenSpec::COLOR_BY_CONTENTS;
    spec->partial_transparency_mode = FlattenSpec::ALLOW_PARTIAL_TRANSPARENCY;
    spec->process_in_memory = 0;
    spec->gimpish_indexed = 1;
}

void add_layer_request(FlattenSpec* spec, const char* layer)
{
...
static enum FlattenSpec::out_color_mode color_by_layers(FlattenSpec* spec)
{
    ...
    else
    {
        ...
        else if (spec->gimpish_indexed && (degrayed == 0 || degrayed == 255))
        {
            grayish = FlattenSpec::COLOR_MONO;
        }
        else
        {
            grayish = FlattenSpec::COLOR_GRAY;

...
#define xcfBE(a) (((uint32_t)xcf_file[(a)] << 24) + ((uint32_t)xcf_file[(a) + 1] << 16) + ((uint32_t)xcf_file[(a) + 2] << 8) + ((uint32_t)xcf_file[(a) + 3]))
#define xcfLE(a) (((uint32_t)xcf_file[(a)]) + ((uint32_t)xcf_file[(a) + 1] << 8) + ((uint32_t)xcf_file[(a) + 2] << 16) + ((uint32_t)xcf_file[(a) + 3] << 24))
...
void computeDimensions(tileDimensions*);

struct _convertParams;
struct xcfTiles
{
    const _convertParams* params;
    uint32_t* tileptrs;
    uint32_t hierarchy;
};

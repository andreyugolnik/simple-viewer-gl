...
static void __ATTRIBUTE__((noreturn)) vFatalGeneric(int status, const char* format, va_list args)
{
    if (format)
    {
        if (*format == '!')
...
void FatalUnsupportedXCF(const char* format, ...)
{
    va_list v;
    va_start(v, format);
    fprintf(stderr, "%s: %s\n ", progname, _("The image contains features not understood by this program:"));
...
{
    if (xcf_file && (uint8_t*)block >= xcf_file && (uint8_t*)block < xcf_file + xcf_length)
        ;
    else
        free(block);
}
...
void closeout(FILE* f, const char* name)
{
    if (f == NULL)
        return;
    if (fflush(f) == 0)
